...
- **Импорт старого lisper.ru (2026-08-22) — план задач** (исходник: `wayback/site/` — 1430 файлов, зеркало с Wayback Machine; инвентарь `wayback/cdx_unique.json`; скрипт `wayback/download.py`):
  ...
  2. **Старый форум**: сначала ПАРСЕР `wayback/site/forum/thread/*.html` (~300 шт.) → структурный формат (threads.json: категории/темы/посты/авторы/даты), импорт в БД — отдельным шагом потом; просмотр на нашем движке по пути /forum-old, ссылка со страницы /forum. **Все импортированные посты принадлежат аккаунту oldlisper**, а оригинальный автор выводится в шапке поста (отдельное поле old_author при импорте)
