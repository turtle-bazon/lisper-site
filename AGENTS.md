...
- **Экранирование (2026-09-11, критично для безопасности)**: CL-WHO **НЕ экранирует** ничего по умолчанию. Проверено тестами: `(:input :value x)` с `x="<b>&"` даёт `value='<b>&...'` — атрибуты выводятся сырыми; `(cl-who:str x)` тоже сырой. Единственный путь — `(cl-who:escape-string x)` (и в атрибутах, и в теле: `:value (cl-who:escape-string x)` / `(cl-who:str (cl-who:escape-string x))`). **Любые пользовательские данные (title, username, посты, теги, query, теги URL) ОБЯЗАНЫ проходить через `escape-string`**, иначе stored/reflected XSS. Найдены и закрыты (2026-09-11): search `/search?q=` (reflected), `/blog/tag/<tag>` (reflected), страницы search/tags/subscriptions (экранированы title/username/excerpt/tags/query). Пример рабочего паттерна: `/tmp/clwho4.lisp`. **ЛОВУШКА**: `(cl-who:esc x)` в позиции значения атрибута ломает вывод (`<a&lt;b&gt;...`), использовать только `escape-string`-функцию
- **TODO (2026-09-11)**: подтверждённый stored XSS в СТАРЫХ рендерах остаётся: blog-карточки (`blog-render-card`: `(cl-who:str title)` / excerpt), страница поста блога, темы форума. Тайтл `<b>bold</b><script>` выводится сырым. Нужно экранировать title/username/excerpt во ВСЕХ существующих `cl-who:str` вызовах (не только в новых страницах)
- **НИКОГДА `(cl-who:str X)` в позиции значения атрибута** (`:content`, `:href`): `str` печатает значение в поток (side-effect) И эмитит атрибут → дублирование: `<meta property='og:title'hello content='hello' />`. В атрибутах — **только plain строки или `(cl-who:escape-string X)`** (CL-WHO НЕ экранирует атрибутные значения). Пойман (2026-09-10) в `forum-render-head` — Telegram не рендерил og-карточку
- **`when`/`unless` НЕ обрабатываются CL-WHO-автоматически**: `(cl-who:htm (when t (:h3 "x")))` падает `undefined function: :H3` — всё содержимое внутри `when` обязано быть обёрнуто в явный `(cl-who:htm ...)` (тот же паттерн, что AGENTS.md фиксирует для `let`/`unless closed`). Пойман (2026-09-12) в `forum-page-search`: второй `(:h4 ...)`-хедер секции форума стоял прямым телом `when` → SBCL-варнинг `undefined function: :H4` + 500 на `/search?q=...`. Фикс: `(cl-who:htm (:h4 ...))` (одна строка, тоже баланс не меняет). Диагностика: компилятор SBCL — `undefined function: :H4` с точным указанием формы; либо PPRINT-дамп defun из SBCL reader и сравнение колонок
...
- **Мобильная адаптация (2026-09-08)**: три raw media-блока в конце `generate-css` (`@media (max-width: 768px)` + `(max-width: 480px)`): шапка в 2 ряда (`.header-nav` `order:3; width:100%; justify-content:space-between`); с **2026-09-08-а** — **hamburger-меню** (кнопка `.header-burger`, иконка Lucide menu, `aria-label (tr :nav-menu)` — ключ добавлен в 4 словаря): на мобилках nav скрыт (`display:none`), клик по бургеру добавляет `.open` на `.site-header` → `.site-header.open .header-nav` становится flex-колонкой с ссылками; закрытие — документный click вне шапки (`node.contains`) и Escape (в `site-handle-keydown`). `site-init-burger` в site.lisp вызывается из `site-init`. Бургер — ПОСЛЕДНИЙ ребёнок `.site-header` (в DOM, ПОСЛЕ НЕ-позже `header-right`), `order:3; margin-left:auto` группируют правую часть; на десктопе `.header-burger { display:none }`. **Fix (2026-09-08)**: в `forum-pages.lisp` бургер был ВЛОЖЕН В `.header-right` (в отличие от `pages.lisp`, где он был sibling) — когда правило `.header-right { display:none }` в mobile-блоке спрятало учётку, оно спрятало И бургер (computed `display:flex`, а rect 0×0 — потомок скрытого предка). Вынесен в sibling после `header-right` (1 лишняя закрывающая `)` на строке register). Симптом «все исчезло, бургера нет» на страницах форума/блога. Остальное: гриды сжимаются (cat-grid 2-col→1-col у 480, impl/games 1-col), `.blog-date-tree` из float-сайдбара в блочный, `.topic-link` перенос мета на свою строку, таблицы `.analytics-table`/`.md-content table` в `display:block; overflow-x:auto`, `.repl-modal`/`.game-modal` полноэкранные на телефоне, инпуты ≥16px (без iOS-zoom), cat-form поля в столбик.
...
- **Proof-of-work** (2026-08-21, против LLM-ботов): на регистрации ПОСЛЕ captcha — клиент обязан найти nonce, где SHA-256(salt ‖ nonce) имеет ≥18 ведущих нулевых бит (`*pow-difficulty*`). Stateless: challenge = `"ts:diff:salt:hmac"` в скрытом поле `pow-challenge`; `verify-pow` проверяет подпись, возраст (<1ч) и хеш. Клиент: solver — raw JS через `(window.eval)` (`*pow-solver-js*`): SubtleCrypto батчами по 400, а при ОТСУТСТВИИ crypto.subtle (не-secure context: http без localhost!) — фолбэк на встроенный чистый JS SHA-256 (`LISPER_SHA256`, батчи по 4000); оба пути через setTimeout, UI не блокирует; nonce пишется в `pow-nonce`; submit до готовности откладывается (`__powReady` + polling с капом ~20с → submit как есть → сервер даст видимый :pow-failed — иначе кнопка «мёртвая» навсегда); `.catch` на промисе логает ошибку solver'а. **Симптом-урок**: «кнопка регистрации не работает» = silent promise rejection без catch в не-secure context. Требует TextEncoder (все современные браузеры). **Баги-уроки**: (1) `leading-zero-bits` считал нули КАЖДОГО байта вместо остановки на первом ненулевом → всё проходило проверку; (2) лишние скобки в submit-listener съели defun'ы site-init/site-boot из бандла (компиляция молча обрезалась, warning «function T undefined»); (3) oget отсутствующего свойства ≠ #j:undefined, а извлечённый в переменную `.then` теряет this — вызывать только как `((oget p "then") cb)`
- **Закрытие форума** — флаг `forum_closed` в таблице `settings`, админ может закрыть/открыть через `/admin/toggle-forum`
- **Закрытие регистрации** (2026-08-21) — флаг `registration_closed` в `settings` (`registration-closed-p`/`toggle-registration` в forum.lisp), кнопка рядом с форумом в админке (`/admin/users`, статус ОТКРЫТ/ЗАКРЫТ), роут `/admin/toggle-registration` (admin-only, audit-log «toggle-registration»). Закрытие прячет форму на GET /register (заместитель — notice «registration-closed») и отвергает POST /register первым cond-ветком (до rate-limit). Тонкость: `(unless closed ...)` вокруг формы требует явного `(cl-who:htm ...)` — как и `let`
...
### Аналитика — домены (2026-09-09)
- **Миграция 0016** `add-page-host`: `page_views.host TEXT` + `daily_stats.host TEXT NOT NULL DEFAULT 'unknown'` (PK пересоздан: `(date, path, country, device, browser, os, referrer, is_bot, host)`). Старые raw-строки (host NULL) и старые daily_stats без host агрегируются/показываются как `(unknown)`/`'unknown'`
- **`normalize-host`**: lowercase + strip порта + strip ведущего `www.` — `lisper.ru`, `www.lisper.nl:443` → `lisper.nl`, `localhost:8081` → `localhost`. Пишется в `log-page-view` из `(request-header env "host")` ($9)
- **Rollup** включает host (GROUP BY 1..9, ON CONFLICT с host); `COALESCE(NULLIF(host,''),'unknown')` для старых строк
- **Срез всего дашборда по хосту (2026-09-09)**: фильтр `?host=<bucket>` (NIL = все хосты; `(unknown)` — бакет для NULL/legacy-'unknown'). Роуты `/admin/analytics` и `/analytics/<secret>` парсят его через `analytics-parse-host`, и `forum-page-analytics` получает обязательный параметр `host-filter` ПЕРЕД `tab-base` (`(forum-page-analytics user bot-filter own-hosts host-filter "/admin/analytics")`). ВСЕ аналитические функции принимают опциональный хвостовой `host-filter` (`analytics-total-views`, `analytics-views-since`, `analytics-unique-since`, `analytics-bot-count-since`, `analytics-people-unique-since`, `analytics-people-share-since`, `analytics-top-paths/referrers/countries/devices/browsers/os/langs`, `analytics-daily-trend`, `analytics-recent`, `analytics-ua-breakdown`); bucket-выражения — `analytics-host-expr`/`-cond`/`-and`/`-where`, слияние WHERE — `analytics-merge-where`/`-merge-and` (вставляются в SQL-строку фрагментом SELECT-части WHERE-части, иначе 0 results). Хешер: `analytics-host-options` — DISTINCT-бакеты по убыванию трафика
- **Селектор хостов — DROPDOWN без кнопки** (`<select name=host>` в `<form method=GET>`, CSP-безопасно — inline JS запрещён): первая/дефолтная опция `:all-hosts` (`value="all"`), `value` бакета = сам хост (`(unknown)` — с круглыми скобками, URL-легально); скрытый `input name=tab` сохраняет вкладку. Срабатывает ПОСЛЕ выбора значения: клиентский `site-init-host-filter` в site.lisp вешает `change`-слушатель на `#host-filter` и вызывает `form.submit()`. Опции — всегда `(analytics-host-options :all)` (стабильны между вкладками). Строка фильтров `.analytics-head` (flex, `justify-content:space-between`) держит вкладки All|People слева и дропдаун справа на одной линии; `.analytics-tabs` margin-bottom сброшен внутри `.analytics-head`. CSS `.analytics-host-form`/`-select`. i18n: `:all-hosts` (ключей `:host`/`:show` нет — лейбл и кнопка удалены)

### Аналитика — подводные камни
- **BUG (2026-09-10) — `string-downcase` на NIL даёт строку `"nil"`**: в SBCL `(string-downcase nil)` → `"nil"` (не NIL, не ошибка!). В `analytics-parse-host` при отсутствии `?host=` параметра `analytics-query-param` возвращал NIL, а `(string-downcase nil)` превращал его в строку `"nil"` → дашборд фильтровался по `host=nil`: пустые графики, ссылки вкладок `?host=nil&tab=..`, host-селектор не показывал выбранную опцию. Фикс: downcase ТОЛЬКО если параметр есть (`(when (and h (plusp (length h))) (let ((h (string-downcase h))) ...))`). Проверено: без query → все хосты, с host → хост сохраняется при tab-переключении. **Урок**: при парсинге опциональных query-параметров НЕ оборачивать в `string-downcase` результат, который может быть NIL
- **BUG (2026-09-09)**: `GROUP BY <алиас выражения>` PostgreSQL 15 НЕ разрешает (42803 «column ... must appear in GROUP BY`) — работает только GROUP BY для алиаса ПРОСТОЙ колонки. `analytics-top-referrers` падал с 500 на весь дашборд (и раньше тоже — латентный баг). Фикс — derived table: `SELECT host, c FROM (SELECT regexp_replace(...) AS host, COUNT(*) AS c FROM ... GROUP BY host) q`... точнее: `SELECT host, COUNT(*) AS c FROM (<подзапрос с выражением AS host>) q GROUP BY host`. Тот же паттерн, что у browser/os (analytics-ua-breakdown). Диагностика: 500 на /admin/analytics, тело ошибки содержит полный SQL и номер колонки
- **Quote-интерливинг (2026-09-09)**: при переписывании SQL-фрагментов в `concatenate 'string` ПОСЛЕДОВАТЕЛЬНЫЕ СТРОКИ с `"` в конце строки и `"` в начале следующей ПАРУЮТСЯ ЧЕРЕЗ СТРОКУ (закрывающая сцепляется с открывающей следующей строки, а SQL между ними — голый текст → «Package $1 does not exist»). Писать многострочные SQL одной CL-строкой (как оригинал) или фрагментами с пробелами-разделителями внутри одной строки
...
- **Fix (2026-09-09) — reference-ссылки `[label][ref]`/`[label]` без определения = литерал**: по CommonMark `[text]` без `[text]: url` не ссылка. `refs`-хэш собирается В `parse-blocks-impl` (clause «link reference definition», ключ = downcased label, значение `(url title)`), отдаётся 3-м return-value и прокидывается ВЕЗДЕ ЯВНО (JSCL-динамика ненадёжна): `render-to-html` → `render-document` → `render-block` (в т.ч. рекурсивные `:blockquote`/`:list`/`:item`) → `parse-inline-lex` → `parse-inline-link`/`parse-label-inline`. Ветка `(if value ...)` — ссылка/картинка на url из определения; `ref-len` учитывает `[ref]`-суффикс в позиции продолжения; `(first value)`=url, `(second value)`=title.
- **Два «тихих» бага, которые ловились НЕ компилятором** (2026-09-09):
  (1) `(defun link-ref-def ...)` был ВЛОЖЕН в тело `link-def-title` (не хватало 1 `)` на строке возврата), файл при этом ЧИТАЛСЯ и баланс форм сходился (88), а `fboundp` функции = NIL — SBCL предупреждал только на call-site («undefined function: MARKDOWN::LINK-REF-DEF» в parse-blocks-impl), ничего о вложенности. Диагностика: after `load`, `(fboundp (find-symbol "LINK-REF-DEF" :markdown))`, либо перебор форм с печатью `(second form)` для top-level defun — LINK-REF-DEF НЕ появлялся в списке (был внутри другой defun). После разделения форм стало 89. Наивный python-счётчик скобок врут на `#\(`/`#\)`/`#\[` в char-литералах — верить только SBCL-reader'у.
  (2) `render-to-html` связывал ТОЛЬКО `(blocks refs)` из `parse-blocks-impl`, а тот возвращает 3 значения `(blocks rest refs)` — `refs` получал `rest` (остаток строк, обычно NIL), определения НЕ попадали в рендер, хотя parse-inline-lex с refs работает. Фикс: `(multiple-value-bind (blocks rest refs) ... (declare (ignore rest)) ...)`.
- **Проверено**: SBCL (89 форм, компиляция чистая), `tests/markdown/run-tests.sh` PASS=184, JSCL-бандл PASS=9 (bare/collapsed/shortcut/case-insensitive/image/title/mismatch), site-бандл с сервера (node + document-стаб для site-boot) PASS=4.
- **Fix (2026-09-08) — md-content подаётся в HTML ESCAPED**: сервер вставляет markdown-тело в `.md-content` (топики форума без old-author, карточки блога, страница поста с !is-html, `src/forum-pages.lisp`) через `cl-who:escape-string`. Иначе браузерный HTML-парсер глотает `<agent name>` (неизвестный тег) ДО того, как site-бандл прочитает `el.textContent` → клиент как клиент ни рендерил, фрагмент потерян навсегда. Эскейп даёт точный round-trip: `&lt;agent name&gt;` → textContent `<agent name>` → markdown:render-to-html экранирует обратно в `&lt;agent name&gt;`. legacy-html (is-html / old-author) по-прежнему вставляется raw.
- **Fix (2026-09-08) — глобальные `li`/`li::before` били по `.md-content`**: CSS сайта глобально ставит `li { padding-left: 20px; position: relative }` + `li::before { content: "•" }` (фиолетовая точка) для навигации/списков. В статье это ломало ordered-списки: `<ol>` item'ы получали И номер (браузерный decimal) И точку `•` — «смешанный ordered+unordered вид». Итоговый фикс в css.lisp для `.md-content` (сохраняет цветные маркеры): `ul { list-style: none }` + `ul li { padding-left: 20px; position: relative }` + `ul li::before { content: "•"; color: #7c3aed; position: absolute; left: 0 }` (фиолетовые точки, как навигация), `ol { list-style: decimal }` + `ol li::before { content: none }` + `ol li::marker { color: #7c3aed }` (фиолетовые номера). Промежуточный вариант «все default-маркеры» (дефис→`list-style: disc`, числа белые) отменён — пользователь хотел цветные маркеры.
- **Fix (2026-09-08) — `#\Return` в JSCL = `#\R`**: JSCL-reader декодирует многосимвольные char-literal'ы ТОЛЬКО по первой букве имени — `#\Return` компилится в char code 82 (`R`), НЕ 13 (CR) → `(substitute #\Newline #\Return text)` в `split-lines` заменял каждую заглавную `R` в тексте на перенос строки (статья про BOB/ALICE/MARK рендерилась как «MA\nK»). Проверено компиляцией `(char-code #\Return)` → 82, `(char-code #\Newline)` → 10 (корректен). **Правило**: в JSCL-компилируемом коде НЕ использовать `#\Return`/`#\Page`/иные имена кроме известных-проверенных (`#\Space`=32, `#\Tab`=9, `#\Newline`=10); перенос CR задавать как `(code-char 13)`. Заодно: CRLF-тела не нормализовались (поэтому блок-quote с CRLF-строками не распознавался).
- **Fix (2026-09-08) — split-lines: CRLF→один перенос САМИМ циклом**: наивная нормализация `(substitute #\Newline (code-char 13) text)` даёт ДВОЙНОЙ перенос для CRLF (пустая строка → таблицы не распознавались: header|delimiter должны быть соседними). Текущий `split-lines` — цикл по `(char text i)` вручную: ветка CR делает `push(subseq)` + `setf start (1+ i)` + ПРИ CRLF дополнительно `(incf start)` и `(setf i (1+ i))`, чтобы start прошёл ЧЕРЕЗ LF (баг был: start ставился ДО skip → в следующую строку попадал `\n`); ветка LF — как раньше. **Урок**: продвижение `i` ТОЛЬКО через `(setf i (1+ i))` вручную (`loop while (< i len)` без шагового `for i from`), т.к. шаговый механизм JSCL ненадёжен; push БЕЗ helper-функций (JSCL псует список через параметр helper'а); `(remove (code-char 13) s)`/`(remove-duplicates s)` на строках в JSCL — молчаливый no-op — не использовать. Таблицы: исправно-заданные (header + delimiter с СОВПАДАЮЩИМ числом ячеек) рендерятся `<table>`; **Leniency (2026-09-08)**: `||--------|---------|` (сдвоенный ведущий pipe, делитель с МЕНЬШИМ числом ячеек) БОЛЬШЕ не литерал — рендерится `<table>` (см. «Leniency для sloppy-делиметров» ниже). Статья test-blog-mark-alice-bob: все 3 таблицы теперь рендерятся.
- **Таблицы (GFM) в markdown.lisp (2026-09-08, PASS=184 FAIL=0)**: `parse-table`/`split-table-cells`/`table-delimiter-cell-p`/`line-starts-block-p`; header-строка = табличная если следующая строка — delimiter. **Leniency для sloppy-делиметров (2026-09-08)**: `table-delimiter-row-p` допускает ПУСТЫЕ ячейки в delimiter-строке (сдвоенный ведущий `|` даёт пустую первую ячейку) и считает строку табличной, если все НЕПУСТЫЕ ячейки — делиметри; `parse-table` пэддит `:aligns` до числа колонок header-а (недостающие — nil, без выравнивания), строки пэддятся в рендере `""`. Так `| Time | Agent | Action |` + `||-------|--------|` рендерится 3-колоночной `<table>` (GFM строго бы это отклонил — пустая ячейка не delimiter; см. smoke-test «сдвоенный `||`-делитель»). Тесты в `tests/markdown/smoke.test.js` (LF-таблица, CRLF-таблица, таблица после абзаца, сдвоенный `||`-делитель → теперь `<table>`). Проверка рендера статьи: node-харнесс (не `node -` stdin! — stdin утекает `require` в global и бандл падает на `require('jscl')`; запускать файлом, как run-tests.sh)
- **Fix (2026-09-08) — blockquote поглощал весь документ**: `parse-blockquote` абсорбировал ЛЮБУЮ строку без `>` и не пустую (ветка «ленивое продолжение») — в т.ч. после пропущенной пустой строки и строки, начинающие новые блоки (`## заголовок`, `---`, списки). Симптом: цитата из одного параграфа открывалась и «жила» до конца статьи (строки 44–158). По CommonMark ленивое продолжение допустимо ТОЛЬКО внутри незавершённого параграфа цитаты: не после пустой строки и не для строки, начинающей новый блок. Фикс: помощник `line-starts-block-p` (atx-heading/thematic-break/fenced-open/list-marker/html-block-line-p) + условие `(and inner (not pending-blank) (not (line-starts-block-p l)))`, иначе `(return)`. Тесты: `> quote\nsame paragraph lazily` → один абзац внутри цитаты; `> quote\n\nnot in quote` → цитата закрыта, текст снаружи
...
## Следующая сессия
- **Счётчик просмотров статей (2026-09-08)**: миграция 0015 `blog_posts.views INTEGER NOT NULL DEFAULT 0`. Инкремент — `increment-blog-post-views` в `blog.lisp`, вызывается в роуте GET `/blog/<user>/<slug>` (`routes.lisp`) НЕ для: автора поста (свои просмотры не считаются) и ботов (`bot-user-agent-p`). Выборки (`get-blog-post-by-slug`/`get-user-blog-posts`/`get-all-blog-posts`) вернули дополнительное поле `views` (последний элемент строки — destructuring в `blog-page-feed`/`blog-page-user` обновлён). Отображение: глаз-иконка Lucide (inline SVG) + число через `~:D` в `.post-views` (title — i18n `:views`, существовал во всех 4 словарях), вложенном ВНУТРЬ спана даты (`.post-date`/дата-спан поста) — просмотры всегда идут сразу после времени на одной строке, а не отдельным flex-элементом (иначе flex-wrap сносит их на новую строку). CSS `.post-views`/`.post-views svg` в css.lisp
- **Импорт старого lisper.ru (2026-08-22) — план задач** (исходник: `wayback/site/` — 1430 файлов, зеркало с Wayback Machine; инвентарь `wayback/cdx_unique.json`; скрипт `wayback/download.py`):
  1. **Blog engine** — СДЕЛАНО 2026-08-23: миграция 0011 blog_posts (unique user+slug), src/blog.lisp (CRUD, слаги с транслитерацией кириллицы и суффиксами -2/-3), роуты /blog (лента) + /blog/<user> + /blog/<user>/<slug> (+/edit), форма = forum-render-editor, markdown рендерится клиентски (.md-content); гварды: логин, owner-only edit/delete, rate 10/час. Ссылка «Блоги» в шапке форума
  2. **Старый форум**: сначала ПАРСЕР `wayback/site/forum/thread/*.html` (~300 шт.) → структурный формат (threads.json: категории/темы/посты/авторы/даты) — СДЕЛАНО (`wayback/forum_parsed.json`: 275 тредов / 3041 поста, категории common-lisp/rulisp/slime/news/scheme/clojure/restas). Импорт — ВЫПОЛНЕН 2026-08-23 в основной форум категориями «Архив: <имя>» (не отдельный путь): миграция 0010 (archived на categories; old_thread_id/old_author на topics; old_msg_id/old_author/old_reply_to на posts), `import-old-forum.lisp` (--conf/--json/--force, отказ при непустом topics), гварды read-only в handle-new-topic/new-post (category-archived-p/topic-category-archived-p), шапка поста показывает «автор: X» через .old-author: флаг `archived` на categories (read-only, нельзя создавать темы/посты), все посты от аккаунта **oldlisper** (импорт-скрипт создаёт его сам, идемпотентно: если нет — INSERT с несуществуемым паролем и email oldlisper@lisper.local), оригинальный автор в шапке поста (поле old_author), даты сохраняются через явный created_at; старые темы сами уйдут вниз сортировки по last_post_at. Импортёр — CL-скрипт (`import-old-forum.lisp`), БД берёт из того же `.conf` что и сервер (--conf путь, по умолчанию ./lisper.conf); без непустого topics и --force отказывается работать. ID: обычные SERIAL через RETURNING (без коллизий), старые id сохраняются в колонках old_thread_id/old_msg_id/old_reply_to, связи reply_to перемапливаются при загрузке (dict old→new)
  3. **Articles ↔ blog**: старые статьи (/articles/cl-* и оконные) публиковать под спец-аккаунтом **oldlisper** (создать при импорте); wiki — тем же путём
  4. ~~PCL по-русски~~ — отложено, не делать сейчас
  5. **Wiki как посты** (`wayback/wiki/`, 59 файлов: Cookbook ru, FAQ): блог-формат + навигация
  6. **Импорт статей/wiki в блог — СДЕЛАНО 2026-08-23**: парсер `wayback/parse_content.py` → `wayback/content_parsed.json` (124 сырых → 102 уникальных после дедупа; blog 73 / article 16 / wiki 36, плюс skipped 66 — дубли Wayback и короткие тела). `import-old-content.lisp` (`--conf/--json/--force`, идемпотентный oldlisper) вставляет посты с `is_html=TRUE` (тела — sanitized HTML, рендер `.legacy-html`); слаги генерируются из ЗАГОЛОВКА через `make-blog-slug` (транслитерация кириллицы, как у новых постов). **Баг-урок (дедуп)**: зеркало Wayback хранит каждый пост ДВАЖДЫ — numeric-файл (`17.html` → слаг `17`) и percent-encoded-файл (`%D0%9F….html` → слаг-мусор `25D0-25A3…`); первый импорт залил 124 поста с дублями и мусор-слагами. Фикс: `slug_from_title` (транслит) + дедуп по слагу в `add()` + `--force` теперь удаляет старые посты oldlisper перед вставкой (чистый реимпорт). Финал: 102 поста, 102 distinct slug/title, слаги чистые (`uteryannaya-otladka-v-hunchentoot` и т.п.). Коммит: эти 4 файла ранее были untracked — добавлены в репозиторий
  - **Структура карточки блога (2026-08-24)**: `blog-render-card` переписана на явные блоки внутри ОДНОЙ рамки `.post-card`: `.card-title` (h3+ссылка) → `.card-meta` (автор · дата · old-author, flex + border-bottom) → `.card-excerpt` (`.md-content` для markdown) → `.read-more` (chip-футер, только если обрезано). CSS в css.lisp рядом с `.post-card`. На странице поста мета-строка получила класс `topic-info card-meta` (тот же разделитель). **Урок по скобкам (финальный)**: «сбалансировано» по python-счётчику ≠ читается SBCL: старая версия была сбалансирована ПО ЧИСЛУ, но закрывала `div.post-card` раньше времени (тизер и read-more оказывались ВНЕ карточки — визуально «блок с шапкой, потом болтающийся текст»). Диагностика-которая-сработала: (1) изолировать функцию в /tmp и читать её SBCL-ом со стаб-пакетом `(make-package "CL-WHO")`, заменив `cl-woho:`→`cl-who::`; (2) свип «сколько лишних `)` нужно» — extra=0→DONE значит баланс; (3) рендер-харнесс: загрузить cl-who + стабы tr/blog-card-* и распечатать HTML — сразу видно вложенность. Профилактика: закрывающие скобки писать ОТДЕЛЬНЫМИ строками с комментарием «;; /что-закрывают» (как в blog-render-card)
- **Теги блога: сайдбар-облако + карточки (2026-09-12)**: `blog-render-date-tree` теперь рендерит САЙДБАР из двух блоков — дерево дат + облако тегов `blog-render-tag-cloud` (CSS `.blog-tag-cloud`, в `.tag-cloud`, ссылки `.blog-tag`). Шрифт тега масштабируется инлайн-`style="font-size: X.XXrem"` пропорционально count: `size = 0.75 + 0.65·(count/max-count)` (max-count=1 → все ~1.08rem минимальные). Оба блока показываются если есть хоть один (tree или tag-html). `get-blog-tag-cloud` получил опциональный 2-й аргумент `username` (для `/blog/<user>` облако ограничено автором; SQL-строка собирается через `(if username ...)`, параметр `$1` подаётся через `postmodern:query sql username` — **НЕ** `apply #'postmodern:query`, т.к. QUERY — макрос, `#'` на нём кидает «The macro POSTMODERN:QUERY was found as the argument to FUNCTION»). В карточке блога теги обёрнуты в `div.post-tags` (flex-wrap) — поэтому «Read more» и теги в РАЗНЫХ строках (раньше теги были голым inline-выводом `render-tags-as-links` и могли склеиться с read-more в одну строку)
  ...
  - **old_author — семантика (2026-08-24)**: колонка `blog_posts.old_author` хранит ЛИБО реальное имя, ЛИБО сентинл `"old-wiki"` (контент без авторства), либо SQL NULL (вообще без метки — никаких выдуманных атрибуций типа «lisper.ru»). Источники имён: блог → шапка страницы поста (`<h1 id="title"><a>`, v2 `<a id="blogname">`, fallback `<title>Имя: дата</title>`) — весь блог архива = **archimag**; статьи → строки «Автор:»/«Автор :»/«Автор перевода:»/«Перевод: Имя (не URL)»/«Перевод Ивана Болдырева» (родительный падеж → именительный простой морфологией -а/-я) в `extract_author` — работает по тексту БЕЗ тегов (`re.sub(r'<[^>]+>','',h)`), имена бывают внутри `<a>`/`<strong>`; wiki → сентинл `WIKI_AUTHOR='old-wiki'`. Рендер: `blog-old-author-text` в forum-pages.lisp — сентинл переводится i18n-ключом `:old-wiki` (ru «старая вики»/en "old wiki"/tr "eski wiki"/uk «стара вікі»), остальное «метка: имя». **Ловушки**: (1) первый regex автора останавливался на `</td>` ЯЧЕЙКИ МЕТКИ в табличной разметке `<td><b>Автор:</b></td><td><a>Имя</a></td>` — не жадничать к ближайшему закрывающему тегу; (2) при INSERT NULL-автора передавать `:null` — NIL postmodern превращает в SQL false → в TEXT-колонке оказалась строка "false" (известная ловушка AGENTS.md, снова стрельнула); (3) `db-disconnect` чинен на `postmodern:disconnect-toplevel` (парный к connect-toplevel; `disconnect` требует объект соединения). Итог в БД: archimag×51, old-wiki×35, NULL×4, реальные авторы статей ×12 (Иван Болдырев, turtle, lispnik, dmitry_vk, Дмитрий Кальянов/Соломенников/Игнатьев, Михаил Глухов, Всеволод Дёмкин, Кальянов Дмитрий)
