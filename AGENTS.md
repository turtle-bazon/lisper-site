...
- **Тренд**: `analytics-daily-trend` — просмотры/день за 30 дней без пропусков дат: `generate_series` + LEFT JOIN UNION (page_views за буферный период + daily_stats за старые), gap-дни заполняются 0, метка `TO_CHAR(d,'DD.MM')`. Рендер — CSS-бары `.trend-chart`/`.trend-bar*` (height % от max)
