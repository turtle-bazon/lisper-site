...
- **Арифметическая CAPTCHA** (2026-08-21, `make-captcha`/`verify-captcha`): «Сколько будет 6 + 5?» — универсально (не Lisp-вопрос: сайт привлекает НЕ-лисперов!). Stateless: ответ спрятан в HMAC-токене `ts:hmac(ts:answer)` (`captcha-token`), проверка = перподписать `ts:ответ-пользователя` и сравнить mac. Только на регистрации; логин защищён rate limit + fts. **CL-WHO-тонкость**: markup внутри runtime-форм (`let`) требует явного `(cl-who:htm ...)` — без него `(:input ...)` эвалюируется как функция («The function :INPUT is undefined»); `multiple-value-bind` в cl-who тоже не работает
...
## Следующая сессия
- **Открытие регистрации (2026-08-21) — сделано**: self-hosted email+пароль без внешней авторизации; антиспам — rate limiting + HMAC токены форм + honeypot + арифметическая CAPTCHA + bot-UA блок + валидация + троттлинг постинга 30с (см. «Антиспам»). Протестировано curl-ом (все ветки: слепой POST, мгновенный сабмит, honeypot, неверная/верная CAPTCHA, bot-UA, дубликаты, валидация, брутфорс-лимиты, троттлинг). Тестовые данные из dev-БД удалены
