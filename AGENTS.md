...
- **Markdown-парсер (2026-08-14/15)**: emphasis переписан и проверен + мод-3 правило + тесты в `tests/markdown/` (см. раздел выше). СДЕЛАНО (2026-08-14): `&quot;`-эскейпинг, Unicode ws/punct, эмфаза в link text, мультистрочные примеры. СДЕЛАНО (2026-08-15): highlight.js УДАЛЁН, подсветка Lisp встроена в `markdown:render-to-html` (`highlight-lisp`, см. «Подсветка синтаксиса Lisp на чистом CL»), CDN-скриптов больше нет. `./tests/markdown/run-tests.sh` PASS=178 FAIL=0, node-тесты site-бандла (/tmp/run_site3.js — ALL PASS, /tmp/run_site_md_built.js — ALL PASS)
