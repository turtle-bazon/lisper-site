...
- **Импорт старого lisper.ru (2026-08-22) — план задач** (исходник: `wayback/site/` — 1430 файлов, зеркало с Wayback Machine; инвентарь `wayback/cdx_unique.json`; скрипт `wayback/download.py`):
  ...
  2. **Старый форум**: сначала ПАРСЕР `wayback/site/forum/thread/*.html` (~300 шт.) → структурный формат (threads.json: категории/темы/посты/авторы/даты) — СДЕЛАНО (`wayback/forum_parsed.json`: 275 тредов / 3041 поста, категории common-lisp/rulisp/slime/news/scheme/clojure/restas). Импорт — в ОСНОВНОЙ форум отдельными категориями «Архив: <имя>» (не отдельный путь): флаг `archived` на categories (read-only, нельзя создавать темы/посты), все посты от аккаунта **oldlisper**, оригинальный автор в шапке поста (поле old_author), даты сохраняются через явный created_at; старые темы сами уйдут вниз сортировки по last_post_at
