...
(defun site-pow-retry-submit (form left)
  ;; Кап ~20с (130×150мс): если solver так и не решил (сломанное
  ;; окружение), отправляем как есть — сервер отдаст видимый :pow-failed.
  (cond
    ((cl-bool (jscl::oget #j:window "__powReady"))
     ((jscl::oget form "submit")))
    ((<= left 0)
     (site-log-error "pow wait timeout, submitting anyway")
     ((jscl::oget form "submit")))
    (t
     ((jscl::oget #j:window "setTimeout")
      (lambda () (site-pow-retry-submit form (1- left)))
      150))))

(defun site-pow-wait-submit (form)
  ;; form.submit() не вызывает submit-событие, рекурсии нет
  (site-pow-retry-submit form 130))
            ...
            (cond
             ...
             (t
              ...
              (handler-case
                  ...
                  (let ((promise (funcall (jscl::oget #j:window "LISPER_POW_SOLVE")
                                          (js-str salt) diff)))
                    ;; Фон: считаем сразу, пока пользователь заполняет поля
                    ((jscl::oget promise "then")
                     (lambda (nonce-js)
                       (site-pow-on-solved nonce-input nonce-js)))
                    ;; ошибка solver'а не должна оставлять кнопку мёртвой навсегда
                    ((jscl::oget promise "catch")
                     (lambda (err-js)
                       (site-log-error "pow solver rejected: "
                                       (cl-str err-js)))))
                (error (e)
